...
import { BUILDINGS, UPGRADES, CONTRACTS, MILESTONES } from "../data/gameData";
...
function BuildingDetail({ comp, money, getCompTemp, onUpgrade, onDemolish }) {
  ...
  const lc = comp.load > 0.85 ? "bad" : comp.load > 0.65 ? "warn" : "good";
  const tc = temp > 55 ? "bad" : temp > 38 ? "warn" : "good";
  const uc = comp.uptime < 0.8 ? "bad" : comp.uptime < 0.95 ? "warn" : "good";
  ...
  return (
    <div style={{ padding: "10px 14px", overflowY: "auto", height: "100%" }}>
      ...
      >
        <div
          style={{
            ...
            height: 32,
            background: "#0a120a",
            border: `2px solid ${BCOLORS[comp.type]}33`,
            display: "flex",
            alignItems: "center",
        ...
        <div>
          <div
            style={{ fontSize: 8, color: BCOLORS[comp.type], letterSpacing: 1 }}
          >
            {d.label.toUpperCase()}
          </div>
          <div style={{ fontSize: 5, color: "#3a5a3a", marginTop: 1 }}>
            {d.desc}
          </div>
        </div>
      </div>
      <InfoRow
        label="LEVEL"
        value={`★ ${comp.level} / 5`}
        valueClass={comp.level >= 5 ? "good" : "warn"}
      />
      <InfoRow
        label="LOAD"
        value={`${(comp.load * 100).toFixed(0)}%`}
        valueClass={lc}
      />
      ...
      <InfoRow
        label="UPTIME"
        value={`${(comp.uptime * 100).toFixed(0)}%`}
        valueClass={uc}
      />

      {comp.level < 5 ? (
        <button
          onClick={() => onUpgrade(comp.id)}
          disabled={!canUp}
          style={{
            marginTop: 8,
            width: "100%",
            ...
            border: `2px solid ${canUp ? "#ffe000" : "#2a3a2a"}`,
            color: canUp ? "#ffe000" : "#2a3a2a",
            cursor: canUp ? "pointer" : "not-allowed",
            letterSpacing: 0.5,
          }}
        >
          ⬆ UPGRADE LV{comp.level + 1} — {fm(upCost)}
        </button>
      ) : (
        <div
          style={{
  ...
  return (
    <div style={{ padding: "8px 12px", overflowY: "auto", height: "100%" }}>
      {UPGRADES.map((upg) => {
        ...
        const reqMet = Object.entries(upg.req).every(
          ([type, cnt]) => comps.filter((c) => c.type === type).length >= cnt,
        );
        const reqLabel = Object.entries(upg.req)
          .map(([t, n]) => `${n}x ${BUILDINGS[t]?.label}`)
          .join(", ");
        ...
        return (
          ...
          >
            ...
            {!owned && (
              <>
                <div style={{ fontSize: 5, color: "#2a4a2a", marginBottom: 4 }}>
                  REQ: {reqLabel}
                </div>
                <button
                  onClick={() => onBuy(upg)}
    ...
    <div style={{ padding: "8px 12px", overflowY: "auto", height: "100%" }}>
      ...
      {activeContracts.length > 0 && (
        <>
          ...
          >
            ACTIVE ({activeContracts.length}/3)
          </div>
          {activeContracts.map((c) => (
            ...
            >
              ...
              >
                <div style={{ fontSize: 7, color: "#44cc77" }}>
                  {c.icon} {c.name}
                </div>
                <div style={{ fontSize: 6, color: "#ffe000" }}>
                  +{fm(c.reward)}
                </div>
              </div>
              <div
              ...
              >
                <div
                  style={{
                    height: "100%",
                    width: `${((c.progress || 0) / c.duration) * 100}%`,
                    background: "#39ff14",
                    transition: "width .5s",
                  }}
                />
              </div>
              <div style={{ fontSize: 5, color: "#2a4a2a", marginTop: 3 }}>
                {Math.round(c.progress || 0)}s / {c.duration}s
              </div>
            </div>
          ))}
        </>
      ...
      <div
        style={{
          ...
          letterSpacing: 1,
          marginBottom: 6,
          marginTop: activeContracts.length ? 8 : 0,
        }}
      >
        AVAILABLE CONTRACTS
      </div>
      {CONTRACTS.map((c) => {
        ...
        const tierColor =
          c.tier === 3 ? "#ff4455" : c.tier === 2 ? "#ffe000" : "#39ff14";
        ...
        return (
          <div
            ...
            style={{
              background: "#0a0f0a",
              border: `1px solid ${canTake ? "#1a3a1a" : "#111811"}`,
              padding: "8px 10px",
              marginBottom: 6,
          ...
          >
            ...
            >
              <div style={{ fontSize: 7, color: "#c0d0c0" }}>
                {c.icon} {c.name}
              </div>
            ...
            >
              <div style={{ fontSize: 5, color: "#ffe000" }}>
                +{fm(c.reward)}
              </div>
            ...
            >
              {c.req.servers && (
                <span style={{ color: srvOk ? "#39ff1466" : "#ff224466" }}>
                  🖥 {c.req.servers} Servers{" "}
                </span>
              )}
              {c.req.gpu && (
                <span style={{ color: gpuOk ? "#39ff1466" : "#ff224466" }}>
                  🎮 {c.req.gpu} GPUs{" "}
                </span>
              )}
              {c.req.storage && (
                <span style={{ color: stgOk ? "#39ff1466" : "#ff224466" }}>
                  💾 {c.req.storage} Storage{" "}
                </span>
              )}
              {c.req.sec && (
                <span style={{ color: secOk ? "#39ff1466" : "#ff224466" }}>
                  🛡 Sec≥{c.req.sec}{" "}
                </span>
              )}
              {
                <span style={{ color: tmpOk ? "#39ff1466" : "#ff224466" }}>
                  🌡 ≤{c.req.temp}°C{" "}
                </span>
              }
              {
                <span style={{ color: upOk ? "#39ff1466" : "#ff224466" }}>
                  ⏱ ≥{Math.round(c.req.uptime * 100)}% up
                </span>
              }
            </div>
            <button
            ...
            >
              {canTake ? "📋 ACCEPT" : "🔒 REQUIREMENTS NOT MET"}
            </button>
          </div>
        );
      })}
...
function MilestonesTab({ achievedMilestones }) {
  const done = MILESTONES.filter((m) =>
    achievedMilestones.includes(m.id),
  ).length;
  return (
    <div style={{ padding: "8px 12px", overflowY: "auto", height: "100%" }}>
      <div style={{ fontSize: 5, color: "#3a5a3a", marginBottom: 6 }}>
        {done} / {MILESTONES.length} COMPLETE
      </div>
      {MILESTONES.map((m) => {
        const owned = achievedMilestones.includes(m.id);
        return (
          <div
            key={m.id}
            style={{
              background: owned ? "#0a1a0a" : "#0a100a",
              border: `1px solid ${owned ? "#ffd70044" : "#1a2a1a"}`,
              padding: "7px 10px",
              marginBottom: 6,
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
              ...
              {owned ? (
                <div style={{ fontSize: 6, color: "#39ff14" }}>
                  ✓ +{m.reward}
                </div>
              ) : (
                <div style={{ fontSize: 6, color: "#3a3a3a" }}>${m.reward}</div>
              )}
            </div>
          </div>
        );
    ...
    >
          ...
          {tabs.map((t) => (
            <button
              ...
              style={{
                ...
                border: "none",
                borderBottom:
                  tab === t.id ? "2px solid #39ff14" : "2px solid transparent",
                color: tab === t.id ? "#39ff14" : "#2a4a2a",
                cursor: "pointer",
                letterSpacing: 0.5,
              }}
            >
        ...
        <div style={{ flex: 1, overflow: "hidden" }}>
          {tab === "info" && (
            <BuildingDetail
              comp={selectedComp}
              money={money}
              getCompTemp={getCompTemp}
              onUpgrade={upgradeComp}
              onDemolish={demolish}
            />
          )}
          {tab === "upgrades" && (
          ...
          {tab === "milestones" && (
            <MilestonesTab achievedMilestones={achievedMilestones} />
          )}
        </div>
      </div>
      ...
      >
        ...
        {(() => {
          ...
          const cpuL = srvs.length
            ? srvs.reduce((a, c) => a + c.load, 0) / srvs.length
            : 0;
          const gpuL = gpus.length
            ? gpus.reduce((a, c) => a + c.load, 0) / gpus.length
            : 0;
          ...
          return [
            {
              n: "CPU LOAD",
              v: cpuL,
              c: cpuL > 0.85 ? "#ff2244" : cpuL > 0.65 ? "#ffe000" : "#39ff14",
            },
            {
              n: "GPU LOAD",
              v: gpuL,
              c: gpuL > 0.85 ? "#ff2244" : gpuL > 0.65 ? "#ffe000" : "#aa55ff",
            },
            {
              n: "AVG TEMP",
              v: tmpN,
            ...
            { n: "SECURITY", v: secN, c: "#ffe000" },
            {
              n: "AVG UPTIME",
              v: uptN,
              c: uptN < 0.8 ? "#ff2244" : uptN < 0.95 ? "#ffe000" : "#39ff14",
            },
          ].map((s) => (
            <div key={s.n} style={{ marginBottom: 5 }}>
              ...
              >
                <div
                  style={{ fontSize: 5, color: "#2a4a2a", letterSpacing: 0.5 }}
                >
                  {s.n}
                </div>
                <div
      ...
      >
        ...
        {gameState.log.slice(0, 20).map((l) => (
          <div
            key={l.id}
            style={{ display: "flex", gap: 4, fontSize: 5, lineHeight: 1.8 }}
          >
